...
func (s *AliyunCMS) Init() error {
	...
	s.measurment = formatMeasurement(s.Project)
...
func (s *AliyunCMS) gatherMetric(acc telegraf.Accumulator, metricName string, metric *Metric) error {
	...
	NextDataPoint:
		for _, datapoint := range datapoints {
			...
			acc.AddFields(s.measurment, fields, tags, time.Unix(datapointTime, 0))
		}
...
func (s *AliyunCMS) prepareTagsAndDimensions(metric *Metric) {
	...
	//Reading all data from buffered chanel
L:
	for {
		select {
		case s.discoveryData = <-s.dt.discDataChan:
			newData = true
			continue
		default:
			break L
