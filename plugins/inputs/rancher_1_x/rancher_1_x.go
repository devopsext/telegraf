...
func (r *Rancher1X) Gather(acc telegraf.Accumulator) error {
	...
	eventsGathered := len (events.Data)
	if eventsGathered > 0 {
		log.Printf("I! [inputs.rancher_1_x] Gathered %d events with timeframe: %s - %s", eventsGathered,previousRun.Format(time.RFC3339),r.previousRun.Format(time.RFC3339))
	}
	...
	for _, event := range (events.Data) {
		...
		if _, ok := r.serviceDict[event.ServiceId]; ok {
			...
			tags["location"] = fmt.Sprintf("%s.%s.%s",tags["environment"],tags["stack"],tags["service"])
